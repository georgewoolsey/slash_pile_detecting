# Quantification Accuracy{#preds_quantit}
...
The slope and intercept of the regression will provide insight into how well the framework represents pile dimensional measurements across the entire range of pile sizes available in the reference data. The coefficient of determination (R^2^) will tell us how well the variability in the reference pile size is explained by the predicted values. In other words, a high R^2^ means that our method for quantifying slash pile form successfully captures the trend of the data: we predict larger piles when reference piles are larger and vice-versa. 
...
```{r, results=F, fig.show='asis', fig.width=10, fig.height=8}
p_temp
```

```{r, results=F, include=F, eval=T}
ggplot2::ggsave("../data/all_comp_scatter.jpeg", plot = p_temp, height = 9.9, width = 8.5, dpi = 300)
```

```{r, results=F, include=F, eval=T}
p_top_left <- diameter_comp_scatter1 + 
  theme(
    plot.background = element_rect(color = "gray88", linewidth = 1, fill = NA)
    # , plot.margin = ggplot2::margin(t = 0.1, r = 0.1, b = 0.1, l = 0.1, unit = "in")
    , plot.title = ggplot2::element_blank()
    , plot.subtitle = ggplot2::element_blank()
  )

p_top_right <- height_comp_scatter1 + 
  theme(
    plot.background = element_rect(color = "gray88", linewidth = 1, fill = NA)
    # , plot.margin = ggplot2::margin(t = 0.1, r = 0.1, b = 0.1, l = 0.1, unit = "in")
    , plot.title = ggplot2::element_blank()
...
p_bottom_group <- wrap_elements(full = (
  (plts_temp[[1]] + plts_temp[[2]]) / (plts_temp[[3]] + plts_temp[[4]]) +
  plot_annotation(
    theme = 
      ggplot2::theme(
        plot.background = element_rect(color = "gray88", linewidth = 1, fill = NA)
...
final_layout <- 
  ...
  p_bottom_group + 
  plot_layout(
    heights = c(0.05, 1, 0.05, 2.1)
  ) +
  plot_annotation(
    ### this is so horrible......
    tag_levels = list(c(
      "                                                 (a) Predicted Diameter versus Reference Diameter"
      , "                                         (b) Predicted Height versus Reference Height"
      , "\n\n(c) Predicted Area versus Reference Area" ## this gd tag is higher than the others?
    ))
  ) & 
  ggplot2::theme(
    axis.title = ggplot2::element_text(size = 7.5)
...
# final_layout
ggplot2::ggsave("../data/all_comp_scatter2_dbscan.jpeg", plot = final_layout, height = 10.5, width = 8.5, dpi = 300)

```

```{r, warning=FALSE, message=FALSE, echo=FALSE, include=FALSE}
remove(list = ls()[grep("_temp",ls())])
remove(diameter_comp_scatter1, height_comp_scatter1, final_layout, p_top_left, p_top_right, p_bottom_group)
gc()
```

```{r, results=F, include=FALSE, eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
################################################################################################################################################
# do it real messy by repeatin allll the code with the watershed
################################################################################################################################################
####################################
# height scatter
####################################
...
height_comp_scatter1 <-
  ...
  ggplot2::annotate(
    ...
    , label = 
      paste0(
        "RMSE = "
        , agg_ground_truth_match_ans %>% 
          dplyr::filter(site_data_lab=="psinf", tolower(method)=="watershed") %>% 
          dplyr::pull(diff_field_height_m_rmse) %>% 
          scales::number(accuracy = 0.01, suffix = " (m)")
        , "\nMAPE = "
        , agg_ground_truth_match_ans %>% 
  ...
  ggplot2::scale_x_continuous(limits = c(0, max( max(watershed_gt_pred_match[["psinf"]]$pred_height_m,na.rm=T), max(watershed_gt_pred_match[["psinf"]]$field_height_m,na.rm=T) )*1.02 )) +
  ggplot2::scale_y_continuous(limits = c(0, max( max(watershed_gt_pred_match[["psinf"]]$pred_height_m,na.rm=T), max(watershed_gt_pred_match[["psinf"]]$field_height_m,na.rm=T) )*1.02 )) +
  ggplot2::facet_wrap(facets = dplyr::vars(site_full_lab), scales = "free", axis.labels = "all") +
  ggplot2::labs(
    x = "reference height (m)" # latex2exp::TeX("reference paraboloid volume $m^3$")
    , y = "predicted height (m)" # latex2exp::TeX("predicted irregular CHM volume $m^3$")
...
diameter_comp_scatter1 <-
  ...
  ggplot2::scale_x_continuous(limits = c(0, max( max(watershed_gt_pred_match[["psinf"]]$pred_diameter_m,na.rm=T), max(watershed_gt_pred_match[["psinf"]]$field_diameter_m,na.rm=T) )*1.02 )) +
  ggplot2::scale_y_continuous(limits = c(0, max( max(watershed_gt_pred_match[["psinf"]]$pred_diameter_m,na.rm=T), max(watershed_gt_pred_match[["psinf"]]$field_diameter_m,na.rm=T) )*1.02 )) +
  ggplot2::facet_wrap(facets = dplyr::vars(site_full_lab), scales = "free", axis.labels = "all") +
  ggplot2::labs(
    x = "reference diameter (m)" # latex2exp::TeX("reference paraboloid volume $m^3$")
    , y = "predicted diameter (m)" # latex2exp::TeX("predicted irregular CHM volume $m^3$")
...
# final_layout
ggplot2::ggsave("../data/all_comp_scatter2_watershed.jpeg", plot = final_layout, height = 10.5, width = 8.5, dpi = 300)
...
## Height Accuracy Investigation{#htpiles_huh}

let's dig into the discrepancy between the predicted and field-measured height values. way back in the [point cloud processing section](#ptcld_process) we noticed that the base of the pile was clearly visible within the DTM of the example pile footprint. This indicates that the lower portion of some piles may be incorrectly classified as ground by the CSF ground classification algorithm. Here, we'll see if we can quantify "how much" of the pile height was included as ground instead of being attributed to the pile vertical dimension for the piles with the largest discrepancy between the predicted and reference height
...
look at the `r n_temp` piles with the largest height under-prediction
...
let's do this for all `r n_temp` piles with the largest predicted height difference
...
**Mean Difference (MD):** 
$$\text{MD} = \frac{1}{N} \sum_{i=1}^{N} (\text{Reference Paraboloid Volume}_i - \text{Predicted Volume}_i)$$
...
we'll dig into the MD shortly but before we move on let's focus on the percent mean difference. We calcualted a %MD of `r scales::percent(vol_agg_df_temp$pct_mean_diff, accuracy = 0.1)` which indicates a major systematic difference where the reference paraboloid volume is, on average, `r scales::percent(vol_agg_df_temp$pct_mean_diff, accuracy = 0.1)` larger than our CHM-predicted irregular CHM volume. This large relative difference shows how much the geometric assumptions inflate the volume compared to the irregular volumes measured by our remote sensing-based method.
...
```{r, include = F, eval = T, message=F, echo=F, warning=F, results=F}
################################################################################################################################################
# do it real messy by repeatin allll the code with the watershed
################################################################################################################################################
# is the mean difference between the two volumes significantly different from zero
...
wshed_lmf_temp <- paste0(
  "y = "
  , scales::number(summary(wshed_lm_temp)$coefficients[2,1], accuracy = 0.01) #slope
  , "x"
  , ifelse(summary(wshed_lm_temp)$coefficients[1,1]<0," - ", " + ")
  , scales::number(abs(summary(wshed_lm_temp)$coefficients[1,1]), accuracy = 0.01) #intercept
)

wshed_volume_comp_scatter1 <-
  watershed_gt_pred_match[["psinf"]] %>% 
  dplyr::filter(match_grp=="true positive") %>% 
  dplyr::bind_cols(
    all_stand_boundary %>% 
  ...
  ggplot2::annotate(
    ...
    , label = 
      ...
      paste0(
        "MD = "
        , scales::comma(wshed_ttest1$estimate, accuracy = 0.01, suffix = " (m³)")
        , "\n"
        , wshed_lmf_temp
        , "\n R² = "
        # , scales::number((summary(lm_temp)$r.squared)*100, accuracy = 0.1, suffix = "%")
        , scales::number((summary(wshed_lm_temp)$r.squared), accuracy = 0.01)
      )
    , hjust = -0.1, vjust = 1.1
    , parse = F
    , size = 3.5
  ) +
  ggplot2::scale_x_continuous(limits = c(0, max( max(watershed_gt_pred_match[["psinf"]]$pred_volume_m3,na.rm=T), max(watershed_gt_pred_match[["psinf"]]$field_volume_m3,na.rm=T) )*1.02 )) +
  ggplot2::scale_y_continuous(limits = c(0, max( max(watershed_gt_pred_match[["psinf"]]$pred_volume_m3,na.rm=T), max(watershed_gt_pred_match[["psinf"]]$field_volume_m3,na.rm=T) )*1.02 )) +
  ggplot2::facet_wrap(facets = dplyr::vars(site_full_lab), scales = "free", axis.labels = "all") +
  ggplot2::labs(
    x = latex2exp::TeX("reference paraboloid volume $m^3$")
    , y = latex2exp::TeX("predicted irregular CHM volume $m^3$")
...
These linear model results (intercept = `r scales::comma(lm_temp$coefficients[1], accuracy = 0.01)`, slope = `r scales::comma(lm_temp$coefficients[2], accuracy = 0.01)`) indicate a strong proportional bias that significantly increases with pile size. The low slope (`r scales::comma(lm_temp$coefficients[2], accuracy = 0.01)`) coupled with the positive intercept (`r scales::comma(lm_temp$coefficients[1], accuracy = 0.01)`) indicate that the volume difference is not a simple constant offset (e.g. slope of ~1.0 and intercept of <0 if our hypothesis of consistently higher reference paraboloid volume is true), but rather a scaling issue that is driven by the largest piles. The much larger reference paraboloid volume estimates relative to the CHM-predicted irregular CHM volumes for the largest piles exert a strong influence on the predicted form of the liner model, pulling the slope steeply downward and forcing the intercept above zero as a mathematical artifact. Despite the predicted positive intercept, visual inspection of the data shows that most reference paraboloid volumes are larger than the CHM-predicted irregular CHM volumes, even for smaller piles. The slope value indicates that for every 1 m^3^ increase in reference paraboloid volume, the predicted irregular volume volume increases by nearly `r scales::comma(lm_temp$coefficients[2], accuracy = 0.01)` m^3^. This data suggests that the geometric assumptions of the paraboloid model potentially introduce substantial scaling error which may limit its reliability (especially for larger piles) for accurately estimating the volume of real-world piles which have heterogeneous footprints and elevation profiles.
...
### Paraboloid Volume Comparison

The volume comparison [immediately above](#vol_comp) compared the reference paraboloid volume using a geometric shape assumption (paraboloid) with the predicted irregular CHM volume based on the irregular CHM elevation profile. Thus, the volume difference included the impact of the shape assumption. In an attempt to remove the influence of comparing a regular shape (field measured) to an irregular shape (predicted irregular CHM volume), we'll perform a comparison analysis where both volume calculations are based on the same paraboloid shape assumption. This should provide insight into the differences related only to the predicted versus field-measured maximum dimensions of height and diameter.
...
we'll dig into the MD shortly but before we move on let's focus on the percent mean difference. We calcualted a %MD of `r scales::percent(vol_agg_df_temp$pct_mean_diff, accuracy = 0.1)` which indicates a slight systematic difference where the reference paraboloid volume is, on average, `r scales::percent(vol_agg_df_temp$pct_mean_diff, accuracy = 0.1)` larger than our predicted allometric volume.
...
That's a lot of plotting to show that the mean difference is `r scales::comma(vol_agg_df_temp$mean_diff, accuracy = 0.01)` m^3^. This is a significant improvement over our comparison between the reference paraboloid value and the irregular, CHM-derived prediction
...
### Paraboloid vs. Paraboloid: Comparison of Individual Piles
...
# summary(wshed_lm_temp)
# scales::percent(summary(wshed_lm_temp)$r.squared, accuracy = 0.1)
# summary(wshed_lm_temp) %>% names()
wshed_lmf_temp <- paste0(
  "y = "
  , scales::number(summary(wshed_lm_temp)$coefficients[2,1], accuracy = 0.01) #slope
  , "x"
...
wshed_volume_comp_scatter2 <-
  ...
  ggplot2::annotate(
    ...
    , label = 
      paste0(
        "MD = "
        , scales::comma(wshed_ttest2$estimate, accuracy = 0.01, suffix = " (m³)")
        , "\n"
        , wshed_lmf_temp
        , "\n R² = "
        # , scales::number((summary(wshed_lm_temp)$r.squared)*100, accuracy = 0.1, suffix = "%")
        , scales::number((summary(wshed_lm_temp)$r.squared), accuracy = 0.01)
      )
    , hjust = -0.1, vjust = 1.1
    , parse = F
...
pltpltplt_temp <- patchwork::wrap_plots(
  list(
    wshed_volume_comp_scatter1 # + ggplot2::labs(subtitle = "A: Predicted Irregular CHM Volume versus Reference Paraboloid Volume")
    , wshed_volume_comp_scatter2 # + ggplot2::labs(subtitle = "B: Predicted Paraboloid Volume versus Reference Paraboloid Volume")
  )
  , nrow = 1
) &
ggplot2::theme(
...
ggplot2::ggsave("../data/volume_comp_scatter_full_watershed.jpeg", plot = pltpltplt_temp, height = 4.8, width = 9, dpi = 300)
```
...
```{r, results=F, include=F, eval=T}
ggplot2::ggsave("../data/all_comp_treatment_total_dbscan.jpeg", plot = p_temp, height = 8, width = 10, dpi = 300)
```
...
wshed_sum_df_temp <-
  ...
  dplyr::bind_rows()
# plot it
wshed_plt_area <- wshed_sum_df_temp %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(
    stand_id=1
    , lab = paste0(
...
wshed_p_temp <-
  patchwork::wrap_plots(
  list(
    wshed_plt_diam + ggplot2::theme(axis.text.x = ggplot2::element_text(size = 8, color = "black", face = "bold"))
    , wshed_plt_ht + ggplot2::theme(axis.text.x = ggplot2::element_text(size = 8, color = "black", face = "bold"))
    # , patchwork::plot_spacer()
    , wshed_plt_area + ggplot2::theme(axis.text.x = ggplot2::element_text(size = 7, color = "black", face = "bold"))
  )
  , design = c(
    # t, l, b , r 
    patchwork::area(1, 1)
...
ggplot2::ggsave("../data/all_comp_treatment_total_watershed.jpeg", plot = wshed_p_temp, height = 8, width = 10, dpi = 300)
```
...
```{r, warning=FALSE, message=FALSE, echo=FALSE, include=FALSE}
remove(list = ls()[grep("_temp",ls())])
remove(plt_area, plt_diam, plt_ht)
gc()
```
