    ...
    , pile_metric = metric %>% 
      ...
      factor(
        ...
        , labels = c(
          ...
          # , "Volume (m3)"
          , "Allometric Field vs\nAllometric Predicted Volume (m3)"
          , "Allometric Field vs\nIrregular Predicted Volume (m3)"
        )
      )
  ) %>% 
...
height_comp_scatter1 <-
  ...
  ggplot2::scale_x_continuous(limits = c(0, max( max(dbscan_gt_pred_match[["psinf"]]$pred_height_m,na.rm=T), max(dbscan_gt_pred_match[["psinf"]]$field_height_m,na.rm=T) ) )) +
  ggplot2::scale_y_continuous(limits = c(0, max( max(dbscan_gt_pred_match[["psinf"]]$pred_height_m,na.rm=T), max(dbscan_gt_pred_match[["psinf"]]$field_height_m,na.rm=T) ) )) +
...
diameter_comp_scatter1 <-
  ...
  ggplot2::scale_x_continuous(limits = c(0, max( max(dbscan_gt_pred_match[["psinf"]]$pred_diameter_m,na.rm=T), max(dbscan_gt_pred_match[["psinf"]]$field_diameter_m,na.rm=T) ) )) +
  ggplot2::scale_y_continuous(limits = c(0, max( max(dbscan_gt_pred_match[["psinf"]]$pred_diameter_m,na.rm=T), max(dbscan_gt_pred_match[["psinf"]]$field_diameter_m,na.rm=T) ) )) +
  ggplot2::facet_wrap(facets = dplyr::vars(site_full_lab), scales = "free", axis.labels = "all") +
  ggplot2::labs(
...
a_temp <- patchwork::wrap_plots(plts_temp, nrow = 1) + 
...
ggplot2::ggsave("../data/all_comp_scatter.jpeg", height = 8, width = 10, dpi = 300)
```

```{r, warning=FALSE, message=FALSE, echo=FALSE, include=FALSE}
remove(list = ls()[grep("_temp",ls())])
remove(diameter_comp_scatter1, height_comp_scatter1)
gc()
```
...
* **predicted piles** 
  - **volume** calculated from the elevation profile of the irregular predicted pile footprint, without assuming a specific geometric shape. we'll refer to this as "Predicted Irregular CHM Volume" to indicate the predicted measurement is from our CHM-based detection methodology
...
# scales::percent(summary(lm_temp)$r.squared, accuracy = 0.1)
volume_comp_scatter1 <- dbscan_gt_pred_match[["psinf"]] %>% 
  dplyr::filter(match_grp=="true positive") %>% 
  ...
  ggplot2::geom_point(color = "navy", alpha = 0.7, size = 2.5) +
  ...
  ggplot2::annotate(
    ...
    , hjust = -0.2, vjust = 1.5, parse = TRUE
  ) +
  ...
  ggplot2::labs(
    ...
    # , color = "image-field\ndiameter diff."
    # , subtitle = latex2exp::TeX("bulk volume ($m^3$) comparison")
    , subtitle = "Predicted Irregular CHM Volume versus Reference Paraboloid Volume"
  ) +
  ggplot2::theme_light()
volume_comp_scatter1
ggplot2::ggsave("../data/volume_comp_scatter1.jpeg", height = 8, width = 8, dpi = 300)
```
...
# scales::percent(summary(lm_temp)$r.squared, accuracy = 0.1)
volume_comp_scatter2 <- dbscan_gt_pred_match[["psinf"]] %>% 
  dplyr::filter(match_grp=="true positive") %>% 
  dplyr::mutate(diff_volume_m3 = field_volume_m3 - pred_allom_volume_m3) %>% 
  ...
  ggplot2::scale_color_viridis_c(option = "mako", direction = -1, alpha = 0.8) +
  ggplot2::scale_x_continuous(limits = c(0, max( max(dbscan_gt_pred_match[["psinf"]]$pred_allom_volume_m3,na.rm=T), max(dbscan_gt_pred_match[["psinf"]]$field_volume_m3,na.rm=T) ) )) +
  ggplot2::scale_y_continuous(limits = c(0, max( max(dbscan_gt_pred_match[["psinf"]]$pred_allom_volume_m3,na.rm=T), max(dbscan_gt_pred_match[["psinf"]]$field_volume_m3,na.rm=T) ) )) +
  ggplot2::labs(
    x = latex2exp::TeX("reference paraboloid volume $m^3$")
    , y = latex2exp::TeX("predicted paraboloid volume $m^3$")
    # , color = "image-field\ndiameter diff."
    # , subtitle = latex2exp::TeX("bulk volume ($m^3$) comparison")
    , subtitle = "Predicted Paraboloid Volume versus Reference Paraboloid Volume"
  ) +
  ggplot2::theme_light()
volume_comp_scatter2
ggplot2::ggsave("../data/volume_comp_scatter2.jpeg", height = 8, width = 8, dpi = 300)
```
...
ggplot2::ggsave("../data/volume_comp_scatter_full.jpeg", height = 6, width = 9, dpi = 300)
```
...
dbscan_gt_pred_match[["psinf"]] %>% 
  ...
  dplyr::summarise(dplyr::across(.cols = dplyr::everything(), ~mean(.x,na.rm=T))) %>% 
  tidyr::pivot_longer(
    cols = -c(field_volume_m3)
  ) %>% 
  dplyr::mutate(
    md = ifelse( 
        name=="pred_volume_m3"
...
sum_df_temp %>% 
  dplyr::mutate(
    pile_metric=dplyr::case_when(
      stringr::str_detect(metric,"_allom_") ~ "allometric"
      , T ~ "irregular"
    )
    , value = scales::comma(value,accuracy=0.1)
    , pct_diff = scales::percent(pct_diff,accuracy=0.1)
  ...
  kableExtra::add_header_above(c(
    " "=2
    , "Allometric Field vs Allometric Predicted Volume (m<sup>3</sup>)" = 3
    , "Allometric Field vs Irregular Predicted Volume (m<sup>3</sup>)" = 3
  ),escape = F) %>% 
  kableExtra::column_spec(seq(2,8,by=3), border_right = TRUE, include_thead = TRUE) %>% 
  # kableExtra::column_spec(
  #   column = 2:8
