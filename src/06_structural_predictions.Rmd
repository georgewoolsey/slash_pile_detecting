...
all_stand_boundary$site_data_lab %>% 
  purrr::map(
    ...
    terra_plt_ortho(
      ortho = rgb_rast[[x]]
      , gt_piles = slash_piles_polys[[x]]
      , pred_piles = dbscan_spectral_preds[[x]]
      , boundary = stand_boundary[[x]]
      , title = paste0(
        "Final DBSCAN: "
        , all_stand_boundary %>% dplyr::filter(site_data_lab==x) %>% dplyr::pull(site)
...
all_stand_boundary$site_data_lab %>% 
  purrr::map(
    ...
    terra_plt_ortho(
      ortho = rgb_rast[[x]]
      , gt_piles = slash_piles_polys[[x]]
      , pred_piles = watershed_spectral_preds[[x]]
      , boundary = stand_boundary[[x]]
      , title = paste0(
        "Final Watershed: "
        , all_stand_boundary %>% dplyr::filter(site_data_lab==x) %>% dplyr::pull(site)
...
let's evaluate how our method performed at correctly predicting the presence of slash piles on the landscape

```{r}
xxx
# ground_truth_prediction_match_ans <- 
  ground_truth_prediction_match(
    ground_truth = slash_piles_polys[["pj"]] %>% 
      sf::st_transform( sf::st_crs(watershed_spectral_preds[["pj"]]) ) %>% 
      dplyr::arrange(desc(image_gt_area_m2)) # this is so the algorithm starts with the largest
    , gt_id = "pile_id"
    , predictions = watershed_spectral_preds[["pj"]]
    , pred_id = "pred_id"
    , min_iou_pct = 0.45
  )
