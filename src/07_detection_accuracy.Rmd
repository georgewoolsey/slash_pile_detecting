...
...but the only reason to do this would be to use `patchwork` to create a tiled figure that combines all of the example areas from the four study sites. since we are already doing manual stuff, we're just going to generate the images using the fast-plotting of `terra`, save them, and then combine them in powerpoint (which will also be not much fun but maybe AI can align since it's AI and all?)
