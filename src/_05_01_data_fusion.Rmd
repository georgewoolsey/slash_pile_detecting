...
## Ground Truth Pile Spectral Summary
...
```{r, include=FALSE, eval=TRUE}
ggplot2::ggsave("../data/pile_tiles_spect_index_filtered.jpeg", height = 8, width = 9)
```
...
we previously worked through an example where we identified candidate slash piles based on their structural form using our raster-based [watershed segmentation approach](#slash_pile_detect_watershed). let's go back to that example and see how the spectral filtering method we defined above impacts the results.
...
let's see how we did with the full data fusion predictions compared to the ground truth data using the instance matching process we outlined in this [earlier section](#iou_match)
