...
```{r, fig.height=9, fig.width=7.5, eval=FALSE}
...
if(!file.exists(rgb_fnm_temp)){
  ...
  # this takes too long...
  psinf_rgb_rast <- change_res_fn(psinf_rgb_rast, my_res=0.06, ofile = rgb_fnm_temp)
}else{
...
ortho_plt_fn = function(rgb_rast, stand, buffer = 10, plt_lwd = 1, plt_line_col = "black"){
  ...
  rgb_df_temp <-
    ...
    dplyr::mutate(
      hex_col = 
        ifelse(
          !is.na(red) & !is.na(green) & !is.na(blue)
          , grDevices::rgb(
            red
            , green
            , blue
...
```{r}
ortho_plt_fn(psinf_rgb_rast, psinf_stand_boundary)
```
