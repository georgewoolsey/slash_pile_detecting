...
# run RGB plotting
# rgb_plotting_switch <- "```{r,eval=F,purl=FALSE"
```
...
all_stand_boundary <-
  ...
  dplyr::mutate(
    ...
    , site_full_lab = dplyr::case_match(
      site
      , "PSINF Mixed Conifer Site" ~ "Pike and San Isabel National Forest (PSINF)"
      , "TRFO-BLM Pinyon-Juniper Site" ~ "Tres Rios Field Office (TRFO-BLM)"
      , "BHEF Ponderosa Pine Site" ~ "Black Hills Experimental Forest (BHEF)"
      , "ARNF Ponderosa Pine Site" ~ "Arapahoe and Roosevelt National Forest (ARNF)"
    )
  )
  # sf::st_centroid() %>% 
  # sf::st_sf() %>% 
...
facts_temp <- "c:/Users/georg/Downloads/Actv_TimberHarvest.gdb/Actv_TimberHarvest.gdb/"
facts_temp <- sf::st_read(
  dsn = facts_temp
  , layer = sf::st_layers(facts_temp)[[1]]
)
facts_temp <- facts_temp %>% 
  dplyr::rename_with(tolower) %>% 
  dplyr::filter(
    tolower(stage_desc) == "accomplished"
    , !is.na(date_completed)
    , date_completed >= lubridate::today() - lubridate::years(8)
  ) %>% 
  ...
  dplyr::filter(!sf::st_is_empty(.)) %>%
  dplyr::filter(sf::st_is_valid(.)) %>%
  sf::st_transform(sf::st_crs(all_stand_boundary))

facts_temp %>% sf::st_drop_geometry() %>% dplyr::count(stage_desc)
dplyr::glimpse(facts_temp)
all_stand_boundary %>% 
  sf::st_centroid() %>% 
  dplyr::select(site) %>% 
  sf::st_intersection(facts_temp)

ft_temp <- "c:/Users/georg/Downloads/Actv_HazFuelTrt_PL.gdb/Actv_HazFuelTrt_PL.gdb/"
ft_temp <- sf::st_read(
  dsn = ft_temp
  , layer = sf::st_layers(ft_temp)[[1]]
)
ft_temp <-
  ft_temp %>% 
  dplyr::rename_with(tolower) %>% 
...
ft_temp <-
  ...
  sf::st_simplify() %>%
  sf::st_transform(sf::st_crs(all_stand_boundary))
ft_temp %>% dplyr::glimpse()
zzz_temp <- 
  all_stand_boundary %>% 
  sf::st_centroid() %>% 
  dplyr::select(site) %>% 
  sf::st_intersection(ft_temp)
zzz_temp %>% 
  sf::st_drop_geometry() %>% 
  dplyr::count(activity)
zzz_temp %>% 
  sf::st_drop_geometry() %>% 
  dplyr::select(site, activity, date_completed)

actv_temp <- "c:/Users/georg/Downloads/Actv_CFLRP_PL.gdb/Actv_CFLRP_PL.gdb/"
actv_temp <- sf::st_read(
  dsn = actv_temp
  , layer = sf::st_layers(actv_temp)[[1]]
)
actv_temp %>% dplyr::glimpse()
actv_temp <-
  actv_temp %>% 
  dplyr::rename_with(tolower) %>% 
  dplyr::filter(
...
# zzz_temp <- 
  ...
  sf::st_centroid() %>% 
  dplyr::select(site) %>% 
  sf::st_intersection(actv_temp)

actv_temp <- "c:/Users/georg/Downloads/Actv_StwrdshpCntrctng_PL.gdb/Actv_StwrdshpCntrctng_PL.gdb/"
actv_temp <- sf::st_read(
  dsn = actv_temp
  , layer = sf::st_layers(actv_temp)[[1]]
)
...
```{r,eval=F,purl=FALSE, fig.height=8, fig.width=6.5}
...
# plt_basemap
plt_basemap2 <- my_ggmap_basemap(
  sf_data = all_stand_boundary %>% dplyr::group_by(site) %>% sf::st_point_on_surface()
  , buffer_box = 190000 # 130000
  , zoom_level = 6 # 7
  ...
  plt_basemap + 
    ggplot2::geom_sf(
      data = all_stand_boundary %>% 
        dplyr::group_by(site_full_lab) %>% 
        sf::st_point_on_surface() %>% 
        sf::st_transform(crs=plt_crs)
      , mapping = ggplot2::aes(fill = site_full_lab)
      , size = 5
      , inherit.aes = F
      , shape = 21
      , color = "gray88"
...
plt2_temp <- plt1_temp +
    ggplot2::geom_sf_label(
      data = all_stand_boundary %>% 
        ...
        sf::st_point_on_surface() %>% 
        sf::st_transform(crs=plt_crs)
      , mapping = ggplot2::aes(label = stringr::str_wrap(site_full_lab, width = 25))
      , size = 2.8
      , hjust = -0.06
      , vjust = -0.1 # 0.5
      , inherit.aes = F
...
ggplot2::ggsave(
  filename = "../data/all_study_site_map_pts.jpg"
  , plot = plt1_temp
  , dpi = "print"
  , height = 8, width = 6.5
)

plt1_temp <- plt_basemap2 + 
    ggplot2::geom_sf(
      data = all_stand_boundary %>% 
        dplyr::group_by(site_full_lab) %>% 
        sf::st_point_on_surface() %>% 
...
ggplot2::ggsave(
  filename = "../data/all_study_site_map_pts2.jpg"
  , plot = plt1_temp
  , dpi = "print"
  , height = 8, width = 6.5
)
...
```{r,eval=F,purl=FALSE, fig.height=8, fig.width=8}
# grDevices::jpeg("../data/psinf_stand_rgb.jpeg", width = 8, height = 8, units = "in", res = 300)
# mar = c(bottom, left, top, right)
terra::plotRGB(psinf_rgb_rast, mar=c(2, 0.5, 3.5, 0.2)) # , stretch="lin"
# add stand boundary
terra::plot(
  psinf_stand_boundary %>% 
    terra::vect() %>% 
...
terra::plot(
  psinf_slash_piles_polys %>% 
    ...
    terra::vect() %>% 
    terra::project(terra::crs(psinf_rgb_rast))
  , add = T, border = "cyan", col = NA, lwd = 1.6
)
...
e_temp <- terra::ext(psinf_rgb_rast)
...
clean_d_temp <- ceiling((terra::ncol(psinf_rgb_rast)*terra::res(psinf_rgb_rast)[1]*0.2)/10)*10
terra::sbar(
  d = clean_d_temp
  # , xy = "bottomleft"
...
```{r,eval=F,purl=FALSE, fig.height=7.2, fig.width=9}
# grDevices::jpeg("../data/pj_stand_rgb.jpeg", width = 9, height = 7.2, units = "in", res = 300)
terra::plotRGB(pj_rgb_rast, mar=c(0.2, 0.2, 1, 0.2)) # , stretch="lin"
# add stand boundary
terra::plot(
  pj_stand_boundary %>% 
    terra::vect() %>% 
...
mtext(
  all_stand_boundary %>% 
    ...
    dplyr::pull(site_full_lab)
  # "Tres Rios Field Office (TRFO-BLM)"
  , side = 3, line = 1.5, cex = 0.9, font = 2
)
# sub title top: "side = 3", line = 1
mtext(
  paste0(
    ...
    dplyr::pull(site_area_ha)
    , " | "
    , pj_slash_piles_polys %>% dplyr::filter(is_in_stand) %>% nrow() %>% scales::comma(accuracy = 1, suffix = " validation slash piles")
  )
  , side = 3, line = 0.5, cex = 0.7
)
# 5. Add scale and north arrow
# ?terra::sbar
# have to define the scale bar distance
clean_d_temp <- ceiling((terra::ncol(pj_rgb_rast)*terra::res(pj_rgb_rast)[1]*0.2)/10)*10
terra::sbar(
  d = clean_d_temp
  , xy = "bottomleft"
  , type = "bar"
...
```{r,eval=F,purl=FALSE, fig.height=6.6, fig.width=10}
# grDevices::jpeg("../data/bhef_stand_rgb.jpeg", width = 10, height = 6.6, units = "in", res = 300)
# mar = c(bottom, left, top, right)
terra::plotRGB(bhef_rgb_rast, mar=c(1.5, 0.5, 0.5, 0.2)) # , stretch="lin"
# add stand boundary
terra::plot(
  bhef_stand_boundary %>% 
    terra::vect() %>% 
...
mtext(
  all_stand_boundary %>% 
    sf::st_drop_geometry() %>% 
    dplyr::filter(site_data_lab == "bhef") %>%
    dplyr::pull(site_full_lab)
  , side = 3, line = 1.5, cex = 0.9, font = 2
)
# sub title top: "side = 3", line = 1
mtext(
  paste0(
    ...
    dplyr::pull(site_area_ha)
    , " | "
    , bhef_slash_piles_polys %>% dplyr::filter(is_in_stand) %>% nrow() %>% scales::comma(accuracy = 1, suffix = " validation slash piles")
  )
  , side = 3, line = 0.5, cex = 0.7
)
# 5. Add scale and north arrow
e_temp <- terra::ext(bhef_rgb_rast)
# 3. Define a position BELOW the minimum Y-coordinate (ymin)
# Subtracting a small value moves it into the bottom margin
scale_x_temp <- e_temp$xmin
scale_y_temp <- e_temp$ymin - ( (e_temp$ymax - e_temp$ymin) * 0.045 ) # 8% below the image
# ?terra::sbar
# have to define the scale bar distance
clean_d_temp <- ceiling((terra::ncol(bhef_rgb_rast)*terra::res(bhef_rgb_rast)[1]*0.2)/10)*10
terra::sbar(
  d = clean_d_temp
  # , xy = "bottomleft"
  , xy = c(scale_x_temp, scale_y_temp)
...
arnf_rgb_rast <- terra::rast(
    ...
    "../data/ARNF_DiamondView_202510/arnf_rgb_rast.tif"
  ) %>% 
  terra::subset(c(1,2,3))
# rename bands
...
```{r,eval=F,purl=FALSE, fig.height=6.7, fig.width=10}
# grDevices::jpeg("../data/arnf_stand_rgb.jpeg", width = 10, height = 6.7, units = "in", res = 300)
# mar = c(bottom, left, top, right)
terra::plotRGB(arnf_rgb_rast, mar=c(0.5, 0.5, 1, 0.2)) # , stretch="lin"
# add stand boundary
terra::plot(
  arnf_stand_boundary %>% 
    terra::vect() %>% 
...
mtext(
  all_stand_boundary %>% 
    sf::st_drop_geometry() %>% 
    dplyr::filter(site_data_lab == "arnf") %>%
    dplyr::pull(site_full_lab)
  # "Arapahoe and Roosevelt National Forest (ARNF)"
  , side = 3, line = 1.5, cex = 0.9, font = 2
)
# sub title top: "side = 3", line = 1
mtext(
  paste0(
    ...
    dplyr::pull(site_area_ha)
    , " | "
    , arnf_slash_piles_polys %>% dplyr::filter(is_in_stand) %>% nrow() %>% scales::comma(accuracy = 1, suffix = " validation slash piles")
  )
  , side = 3, line = 0.5, cex = 0.7
)
# 5. Add scale and north arrow
e_temp <- terra::ext(arnf_rgb_rast)
# 3. Define a position BELOW the minimum Y-coordinate (ymin)
# Subtracting a small value moves it into the bottom margin
scale_x_temp <- e_temp$xmin
scale_y_temp <- e_temp$ymin - ( (e_temp$ymax - e_temp$ymin) * 0.035 ) # 8% below the image
# ?terra::sbar
# have to define the scale bar distance
clean_d_temp <- ceiling((terra::ncol(arnf_rgb_rast)*terra::res(arnf_rgb_rast)[1]*0.2)/10)*10
terra::sbar(
  d = clean_d_temp
  # , xy = "bottomleft"
  , xy = c(scale_x_temp, scale_y_temp)
...
```{r,eval=F,purl=FALSE, results=F, warning=FALSE, message=FALSE}
...
```{r,eval=F,purl=FALSE, fig.height=9.5, fig.width=5}
patchwork::wrap_plots(
