...
p1_temp <- slash_pile_detect_watershed_ans_temp$segs_sf %>% 
  ...
  ggplot2::scale_fill_distiller(palette = "cyans", direction = 1) +
  ggplot2::labs(x="",y="") +
  ggplot2::theme_light() +
...
# Caused by error:
# ! Cannot segment the trees from a raster stored on disk. Use segment_trees() or load the raster in memory
class(chm_rast[["bhef"]])
terra::inMemory(chm_rast[["bhef"]])
cloud2trees:::split_raster_fn(chm_rast[["bhef"]])
chm_rast[["bhef"]] %>% terra::plot()

#### modified get_segmentation_candidates to only return slice raster...ran it
xxx <- get_segmentation_candidates(
  chm_rast[["bhef"]],method = "watershed"
  , min_ht_m = all_stand_boundary %>% dplyr::filter(site_data_lab=="bhef") %>% dplyr::pull(min_ht_m)
  , max_ht_m = all_stand_boundary %>% dplyr::filter(site_data_lab=="bhef") %>% dplyr::pull(max_ht_m)
  , min_area_m2 = all_stand_boundary %>% dplyr::filter(site_data_lab=="bhef") %>% dplyr::pull(min_area_m2)
  , max_area_m2 = all_stand_boundary %>% dplyr::filter(site_data_lab=="bhef") %>% dplyr::pull(max_area_m2)
)

terra::inMemory(xxx)
cloud2trees:::split_raster_fn(xxx)
seg_params_ans <- get_segmentation_params(
      min_ht_m = all_stand_boundary %>% dplyr::filter(site_data_lab=="bhef") %>% dplyr::pull(min_ht_m)
      , max_ht_m = all_stand_boundary %>% dplyr::filter(site_data_lab=="bhef") %>% dplyr::pull(max_ht_m)
      , min_area_m2 = all_stand_boundary %>% dplyr::filter(site_data_lab=="bhef") %>% dplyr::pull(min_area_m2)
...
slash_pile_detect_ans <- slash_pile_detect(
    chm_rast = chm_rast[["bhef"]]
    , seg_method = "watershed"
    , min_ht_m = all_stand_boundary %>% dplyr::filter(site_data_lab=="bhef") %>% dplyr::pull(min_ht_m)
    , max_ht_m = all_stand_boundary %>% dplyr::filter(site_data_lab=="bhef") %>% dplyr::pull(max_ht_m)
    , min_area_m2 = all_stand_boundary %>% dplyr::filter(site_data_lab=="bhef") %>% dplyr::pull(min_area_m2)
    , max_area_m2 = all_stand_boundary %>% dplyr::filter(site_data_lab=="bhef") %>% dplyr::pull(max_area_m2)
    , min_convexity_ratio = all_stand_boundary %>% dplyr::filter(site_data_lab=="bhef") %>% dplyr::pull(min_convexity_ratio)
    , min_circularity_ratio = all_stand_boundary %>% dplyr::filter(site_data_lab=="bhef") %>% dplyr::pull(min_circularity_ratio)
  )

# Process CHM with memory-aware tiling and edge-effect resolution
process_chm_segmented <- function(chm_path, tile_size = 500, buffer_size = 20) {
  
  # 1. Argument Validation
  if (base::is.character(chm_path)) {
    if (!base::file.exists(chm_path)) {
      base::stop("The provided file path does not exist.")
    }
    chm_ref <- terra::rast(chm_path)
  } else if (base::inherits(chm_path, "SpatRaster")) {
    chm_ref <- chm_path
  } else {
    base::stop("Input 'chm_path' must be a valid file path string or a SpatRaster object.")
  }

  # 2. Memory Check and Direct Processing
  # If the raster is already in memory, process directly to avoid tiling overhead
  if (terra::inMemory(chm_ref)) {
    base::message("Raster is already in memory. Processing directly with lidR::watershed().")
    return(lidR::watershed(chm_ref))
  }

  # 3. Disk-based Tiling Setup
  base::message("Raster is on disk or too large. Initiating tiled processing...")
  tile_dir <- base::tempfile("tiles_")
  base::dir.create(tile_dir)
  
  # Generate buffered tiles on disk
  tile_files <- terra::makeTiles(
    x = chm_ref, 
    y = tile_size, 
    filename = base::file.path(tile_dir, "tile_.tif"), 
    buffer = buffer_size,
    overwrite = TRUE
  )

  # 4. Iterative Processing of Each Tile
  all_polygons_list <- purrr::map(tile_files, function(t_path) {
    # Load tile
    tile_rast <- terra::rast(t_path)
    
    # Run watershed algorithm
    seg_rast <- lidR::watershed(tile_rast)
    
    # Convert segments to polygons to preserve "whole" tree shapes
    # values = TRUE ensures we keep the ID, aggregate = TRUE merges same-ID pixels
    seg_poly <- terra::as.polygons(seg_rast, values = TRUE, aggregate = TRUE)
    
    if (base::nrow(seg_poly) == 0) return(NULL)
    
    # Calculate core extent to identify segments touching the buffer
    e <- terra::ext(tile_rast)
    core_ext <- terra::ext(
      e$xmin + (buffer_size * terra::res(tile_rast)[1]), 
      e$xmax - (buffer_size * terra::res(tile_rast)[1]), 
      e$ymin + (buffer_size * terra::res(tile_rast)[2]), 
      e$ymax - (buffer_size * terra::res(tile_rast)[2])
    )
    
    # Check if polygons are strictly within the core area
    # If not, they are 'boundary segments'
    is_within <- terra::relate(seg_poly, terra::as.polygons(core_ext), "within")
    
    # Store metadata for resolution logic
    seg_poly$on_buffer <- !base::as.vector(is_within)
    seg_poly$poly_area <- terra::expanse(seg_poly)
    seg_poly$tile_source <- base::basename(t_path)
    
    return(seg_poly)
  })

  # 5. Handle Overlaps using "Largest Area" Logic
  base::message("Consolidating segments and resolving overlaps...")
  
  # Combine all tile results into one vector collection
  all_vec <- terra::vect(purrr::compact(all_polygons_list))
  
  # Separate 'Hold Out' (interior) from 'Buffer' (boundary) segments
  interior_segs <- all_vec[!all_vec$on_buffer]
  boundary_segs <- all_vec[all_vec$on_buffer]
  
  if (base::nrow(boundary_segs) > 1) {
    # Find overlapping polygons in the boundary set
    # terra::pairs returns indices of geometries that intersect
    overlap_idx <- terra::pairs(boundary_segs, intersect = TRUE)
    
    if (base::nrow(overlap_idx) > 0) {
      to_remove <- base::c()
      
      for (i in 1:base::nrow(overlap_idx)) {
        idx1 <- overlap_idx[i, 1]
        idx2 <- overlap_idx[i, 2]
        
        # Compare areas: flag the smaller one for removal
        if (boundary_segs$poly_area[idx1] >= boundary_segs$poly_area[idx2]) {
          to_remove <- base::c(to_remove, idx2)
        } else {
          to_remove <- base::c(to_remove, idx1)
        }
      }
      
      # Remove duplicates and smaller partial segments
      boundary_segs <- boundary_segs[-base::unique(to_remove)]
    }
  }

  # 6. Final Recombination and Rasterization
  # Re-merge the kept boundary segments with the interior segments
  final_vec <- terra::rbind(interior_segs, boundary_segs)
  
  # Assign new unique IDs to the final segments
  final_vec$final_id <- 1:base::nrow(final_vec)
  
  base::message("Creating final output raster...")
  ...
  # Cleanup
  base::unlink(tile_dir, recursive = TRUE)
  
  return(final_raster)
}

# Example Usage:
# result_raster <- process_chm_segmented("path/to/chm.tif")
