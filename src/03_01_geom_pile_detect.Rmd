...
| :--- | :--- | :--- | :--- | :--- |
| **Watershed** | `tol` | Minimum height difference to distinguish objects. | `(max_ht_m - min_ht_m) * 0.5` | Adapts vertical sensitivity to the "range of x," preventing minor height fluctuations from creating false objects. |
...
| **DBSCAN** | `eps` | Maximum distance to consider points as neighbors. | `min(1.5 * (1/sqrt(pts_per_m2)), (sqrt(min_area_m2 / pi) * 0.25))` | Bridges gaps between points while capping the radius at 25% of the target diameter to ensure objects remain spatially separated. |
| **DBSCAN** | `minPts` | Minimum points required to form a cluster core. | `round(total_exp_pts * (eps^2 / target_radius^2))` | Scales the required point "mass" so that core points must meet the density profile of the actual target object. |
