...
get_tile_size <- function(
  rast
  , memory_risk = 0.15
  # low risk (0.01 – 0.05): creates more tiles but is unlikely to crash R
  # med risk (0.1-0.2): reserves ~95% of free RAM for the remaining calculations
  # high risk (0.2-0.5): fewer tiles but more likely to crash R
  # dangerous (>0.5): very likely to crash R
){
...
get_segs_watershed <- function(
  ...
  # tiling setup
  message("Raster is on disk or too large. Initiating tiled processing...")
  ...
  tile_size <- get_tile_size(chm_rast, memory_risk = 0.7)[["tile_size"]]
  ...
  all_polygons_list <- purrr::map(
    ...
    , function(t_path) {
      ...
      if(nrow(segs_poly) == 0){return(NULL)}
      
      # identify segments touching the buffer
      ...
      segs_poly$tile_source <- basename(t_path) # tile id
  ...
  if (nrow(boundary_segs) > 0) {
    # intersection matrix
    adj_matrix <- terra::relate(boundary_segs, boundary_segs, relation = "intersects")
    
    # count how many things each segment intersects (including itself)
    # a count of 1 means it only intersects itself (no olap)
    intersect_counts <- rowSums(adj_matrix)
    
    # segments with no intersections in the buffer area
    solo_boundary_segs <- boundary_segs[intersect_counts == 1]
    # terra::plot(solo_boundary_segs)
    
    # segments that overlap with others
    olap_segs <- boundary_segs[intersect_counts > 1]
    # terra::plot(olap_segs)
    done_olap_ids <- c()
    
    if(nrow(olap_segs) > 0){
      # terra::relate only the olaps
      conf_adj <- terra::relate(olap_segs, olap_segs, relation = "intersects")
      already_processed <- rep(FALSE, nrow(olap_segs))
      
      for (i in 1:nrow(olap_segs)) {
        if (already_processed[i]){next}
        
        # index of the overlapping group
        cluster_idx <- which(conf_adj[i, ] == TRUE)
        
        # take the max area within the cluster
        max_area_idx <- which.max(olap_segs$poly_area[cluster_idx])[1]
        winner_id <- olap_segs$temp_id[cluster_idx][max_area_idx]
        
        done_olap_ids <- c(done_olap_ids, winner_id) %>% unique()
        already_processed[cluster_idx] <- TRUE
      }
      
      kept_olap_segs <- olap_segs[olap_segs$temp_id %in% done_olap_ids]
      
      # bind solo with processed
      boundary_segs <- rbind(solo_boundary_segs, kept_olap_segs)
    }else{
      boundary_segs <- solo_boundary_segs
    }
  }

  # put everything back together and rasterize
...
# xxx_temp <- terra::clamp(bhef_chm_rast, upper = 6, lower = 0, values = F) %>% 
