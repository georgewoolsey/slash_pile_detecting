...
xxx before we filter for area, we should combine segments that are touching...

let's remove all candidate segments that do not meet the area criteria and simplify segments that are disconnected by keeping only the largest portion of the segment based on area. to do this, we'll convert the segmented raster into vector form for area calculations.

```{r}
xxx
watershed_segs_sf <- watershed_segs %>% 
  terra::as.polygons(round = F, aggregate = T, values = T, extent = F, na.rm = T) %>% 
  setNames("pred_id") %>%
  sf::st_as_sf() %>% 
  sf::st_simplify() %>% 
