...
get_segs_watershed <- function(
  ...
  if(is.character(chm_rast) && stringr::str_ends(chm_rast,"\\.tif$|\\.tiff$")) {
  ...
  # tile size, dynamically breaks the raster into 4 regions...might not work for realllllly huge rasters
  tile_size <- c(
      round(terra::nrow(chm_rast)/4)
      , round(terra::ncol(chm_rast)/4)
    )
  tile_radius <- ceiling( sqrt(prod(tile_size)/pi) ) # tile radius
  ...
  buffer_size <- min(
    ...
    , round(tile_radius/terra::res(chm_rast)[1])
  )
...
# ?lidR::watershed
# get_segs_watershed(chm_rast = aoi_chm_rast, tol = 1, ext = 1) %>% terra::plot()
